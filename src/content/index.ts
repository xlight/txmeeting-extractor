...
console.log('[TXMeeting Content] Content script 开始执行，时机:', document.readyState);
  ...
  chrome.runtime.sendMessage(
    ...
    (response) => {
      if (chrome.runtime.lastError) {
        console.error('[TXMeeting Content] 发送失败:', chrome.runtime.lastError);
      } else {
        console.log('[TXMeeting Content] ✅ 数据已发送到 Background:', response);
      }
    }
  );
});
