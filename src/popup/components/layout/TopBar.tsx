...
export function TopBar() {
  ...
  const handleExport = () => {
    ...
    // 获取会议开始时间和标题
    const startTime = meetingData.metadata.start_time || Date.now();
    const dateStr = formatDate(startTime);
    const title = meetingData.metadata.title || '会议记录';

    // 生成文件名: YYYY-MM-DD_会议标题.md
    a.download = `${dateStr}_${title}.md`;
    a.click();
